# CURRENT — load this first

## Live position

- Wed: Fragment **004** secured  
- **Today Thu:** Post 1 — decryption attempted → Terminal corruption / classified breach  
- Post 2 later: Broken named + Archive opening (four types live there, not Terminal)  
- **Mint / Activation: Saturday Aug 1**

## Voice rules

- Clear outgoing transmission · flowing lines · no one-word staccato stacks  
- Caption = story event · don’t hard-CTA “enter the Terminal” · link is enough  
- Four forces reveal = Archive / Post 2, not Terminal type dump

---

## THURSDAY POST 1 — LOCKED (272 chars)

```text
FRAGMENT 004 // DECRYPTION ATTEMPTED

We shouldn't have tried reading it.
The Terminal is in danger.

Too much is coming through.
Hunters need to access the Terminal and lock it down.

Reply ALPHA CONTAINED if you still have access.

http://HoodLynx.xyz
FOLLOW THE SIGNAL.
```

**Job:** Regret + danger · access Terminal to lock it down · reply `ALPHA CONTAINED if you still have access`.  
**Art:** Terminal screens glitching with half-readable Activation crumbs. Clean HD, bright enough for feed.

---

## TERMINAL — corrupted breach (ship with Post 1)

Feels like clearance failed and classified output is unprotected. No four-type roster here.

### Opening screen

```text
// HOODLYNX ARCHIVE TERMINAL
STATUS: CORRUPTED
ARCHIVE: 001 + 002 + 003 STORED
FRAGMENT 004: DECRYPTION UNSTABLE
----------------------------------------
ERROR: CONTAINMENT BREACH
CLEARANCE PROTOCOL: SUSPENDED
CLASSIFIED FILES: UNPROTECTED
----------------------------------------
ACTIVATION: SATURDAY
VENUE: GRAVEMINT.IO
SUPPLY: 4663
CHAIN: ROBINHOOD
----------------------------------------
PHASES:
AUTHORIZED HUNTER ACCESS
FRAGMENT ACCESS
PUBLIC
----------------------------------------
OUTPUT UNCONTAINED
▸ RECONNECT ANYWAY
```

### After submit — NEW

```text
...RECONNECT SIGNAL SENT
...WALLET LOCKED TO IDENTITY
HUNTER STANDING: SIGNAL DETECTED
----------------------------------------
BREACH WINDOW CAUGHT YOUR SIGNAL
AUTHORIZED HUNTER ACCESS: GRANTED
GTD SLOT: QUEUED
----------------------------------------
ACTIVATION: SATURDAY
GRAVEMINT.IO
----------------------------------------
WARNING: TERMINAL SHUTTING DOWN
CLASSIFIED CHANNEL CLOSING
FOLLOW THE SIGNAL.
```

### After submit — RETURNING

```text
...RECONNECT SIGNAL SENT
...RETURNING IDENTITY RECOGNIZED
HUNTER STANDING: RISING
----------------------------------------
BREACH WINDOW CAUGHT YOUR SIGNAL
AUTHORIZED HUNTER ACCESS: GRANTED
GTD SLOT: QUEUED
----------------------------------------
ACTIVATION: SATURDAY
GRAVEMINT.IO // 4663
----------------------------------------
WARNING: TERMINAL SHUTTING DOWN
CLASSIFIED CHANNEL CLOSING
FOLLOW THE SIGNAL.
```

Anyone who gets through during the breach gets the GTD scoop, then the Terminal dies for this beat.

---

## POST 2 direction (not locked yet)

Broken named · Archive opening · four forces / Seekers’ record on site.  
004 gave information; the cost was the system tearing open.

## Fri / Sat

Fri: clean public Activation protocols  
Sat: mint live  
